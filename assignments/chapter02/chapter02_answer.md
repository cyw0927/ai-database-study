...
```text
실행 성공 / 실패: 아직 실행 확인 필요
오류 메시지에서 확인한 핵심 단어: duplicate key / primary key / unique constraint 예상
왜 실패했다고 생각하는가: 같은 PK 값은 한 테이블에서 두 행이 동시에 사용할 수 없기 때문이다.
```
...
```text
실행 성공 / 실패: 아직 실행 확인 필요
오류 메시지에서 확인한 핵심 단어: foreign key 예상
왜 실패했다고 생각하는가: 존재하지 않는 students.id를 enrollment가 참조할 수 없기 때문이다.
```
