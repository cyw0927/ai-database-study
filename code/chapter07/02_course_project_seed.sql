...
BEGIN
    ...
    IF current_setting('transaction_read_only')::boolean THEN
        RAISE EXCEPTION '샘플 입력 중단: 현재 연결이 읽기 전용입니다.';
    END IF;
    ...
    IF to_regclass('course_project.students') IS NULL
       ...
       OR to_regclass('course_project.uq_course_enrollments_active') IS NULL THEN
        RAISE EXCEPTION '샘플 입력 중단: 01_course_project_schema.sql 실행 결과를 확인하세요.';
    END IF;
    ...
    IF v_student_count <> 0 OR v_instructor_count <> 0 OR v_course_count <> 0 OR v_enrollment_count <> 0 THEN
        RAISE EXCEPTION
...
ALTER TABLE course_project.students ALTER COLUMN id RESTART WITH 104;
ALTER TABLE course_project.instructors ALTER COLUMN id RESTART WITH 203;
ALTER TABLE course_project.courses ALTER COLUMN id RESTART WITH 304;
ALTER TABLE course_project.enrollments ALTER COLUMN id RESTART WITH 1005;
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_id_1005_count FROM course_project.enrollments WHERE id = 1005;

    IF v_student_count <> 3 OR v_instructor_count <> 2 OR v_course_count <> 3 OR v_enrollment_count <> 4
       OR v_recorded_total <> 470000 OR v_student_101_enrollments <> 2 OR v_course_301_enrollments <> 2
       OR v_instructor_201_courses <> 2 OR v_active_duplicate_count <> 0
       OR v_status_1001 IS DISTINCT FROM '수강중' OR v_status_1004 IS DISTINCT FROM '신청'
       OR v_id_1005_count <> 0 THEN
        RAISE EXCEPTION
            ...
            v_student_101_enrollments,v_course_301_enrollments,v_instructor_201_courses,v_active_duplicate_count,
            v_status_1001,v_status_1004,v_id_1005_count;
    END IF;
...
SELECT 'students' AS object_name, COUNT(*) AS row_count FROM course_project.students
UNION ALL SELECT 'instructors', COUNT(*) FROM course_project.instructors
UNION ALL SELECT 'courses', COUNT(*) FROM course_project.courses
UNION ALL SELECT 'enrollments', COUNT(*) FROM course_project.enrollments
