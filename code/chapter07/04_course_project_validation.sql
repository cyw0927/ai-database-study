...
BEGIN
    IF current_database() <> 'ai_database_book' THEN
        RAISE EXCEPTION '검증 중단: 현재 데이터베이스는 %입니다. ai_database_book에 연결하세요.', current_database();
    END IF;
    ...
    IF to_regclass('course_project.students') IS NULL
       ...
       OR to_regclass('course_project.uq_course_enrollments_active') IS NULL THEN
        RAISE EXCEPTION '검증 중단: Chapter 07 프로젝트 객체가 모두 존재하지 않습니다.';
    END IF;
    ...
    INTO v_enrollment_count, v_total_recorded, v_non_cancelled_count, v_non_cancelled_recorded
    ...
    SELECT COUNT(*) INTO v_student_101_enrollments FROM course_project.enrollments WHERE student_id = 101;
    SELECT COUNT(*) INTO v_course_301_enrollments FROM course_project.enrollments WHERE course_id = 301;
    SELECT COUNT(*) INTO v_instructor_201_courses FROM course_project.courses WHERE instructor_id = 201;
    ...
    WHERE name IS NULL OR email IS NULL OR joined_at IS NULL
       OR char_length(trim(name)) = 0 OR char_length(trim(email)) = 0;

    SELECT COUNT(*) INTO v_invalid_instructor_count
    FROM course_project.instructors
    WHERE name IS NULL OR email IS NULL OR specialty IS NULL
       OR char_length(trim(name)) = 0 OR char_length(trim(email)) = 0 OR char_length(trim(specialty)) = 0;

    SELECT COUNT(*) INTO v_invalid_course_count
    FROM course_project.courses
    WHERE instructor_id IS NULL OR title IS NULL OR level IS NULL OR price IS NULL OR opened_at IS NULL
       OR char_length(trim(title)) = 0 OR level NOT IN ('basic', 'intermediate', 'advanced') OR price < 0;

    SELECT COUNT(*) INTO v_invalid_enrollment_count
    FROM course_project.enrollments
    WHERE student_id IS NULL OR course_id IS NULL OR enrolled_at IS NULL OR status IS NULL OR recorded_amount IS NULL
       OR status NOT IN ('신청', '수강중', '완료', '취소') OR recorded_amount < 0;
    ...
    SELECT status, recorded_amount INTO v_status_1001, v_amount_1001 FROM course_project.enrollments WHERE id = 1001;
    SELECT status, recorded_amount INTO v_status_1004, v_amount_1004 FROM course_project.enrollments WHERE id = 1004;
    SELECT status, recorded_amount INTO v_status_1005, v_amount_1005 FROM course_project.enrollments WHERE id = 1005;

    IF v_named_constraint_count <> 15 OR v_not_null_count <> 20
       OR v_student_count <> 3 OR v_instructor_count <> 2 OR v_course_count <> 3 OR v_enrollment_count <> 5
       OR v_service_join_count <> 5 OR v_student_101_enrollments <> 2 OR v_course_301_enrollments <> 2 OR v_instructor_201_courses <> 2
       OR v_orphan_student_count <> 0 OR v_orphan_course_count <> 0 OR v_orphan_instructor_count <> 0
       OR v_invalid_student_count <> 0 OR v_invalid_instructor_count <> 0 OR v_invalid_course_count <> 0 OR v_invalid_enrollment_count <> 0
       OR v_active_duplicate_count <> 0
       OR v_status_1001 IS DISTINCT FROM '완료' OR v_amount_1001 IS DISTINCT FROM 100000::numeric
       OR v_status_1004 IS DISTINCT FROM '취소' OR v_amount_1004 IS DISTINCT FROM 150000::numeric
       OR v_status_1005 IS DISTINCT FROM '신청' OR v_amount_1005 IS DISTINCT FROM 120000::numeric
       OR v_total_recorded <> 590000 OR v_non_cancelled_count <> 4 OR v_non_cancelled_recorded <> 440000 THEN
        RAISE EXCEPTION '검증 실패: 최종 기준 상태가 일치하지 않습니다.';
    END IF;
