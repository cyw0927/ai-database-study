...
CREATE TABLE course_project.students (
    ...
    CONSTRAINT chk_course_students_name_not_blank CHECK (char_length(trim(name)) > 0),
    CONSTRAINT chk_course_students_email_not_blank CHECK (char_length(trim(email)) > 0)
);
...
CREATE TABLE course_project.instructors (
    ...
    CONSTRAINT chk_course_instructors_name_not_blank CHECK (char_length(trim(name)) > 0),
    CONSTRAINT chk_course_instructors_email_not_blank CHECK (char_length(trim(email)) > 0),
    CONSTRAINT chk_course_instructors_specialty_not_blank CHECK (char_length(trim(specialty)) > 0)
);
...
CREATE TABLE course_project.courses (
    ...
    CONSTRAINT fk_course_courses_instructor FOREIGN KEY (instructor_id)
        REFERENCES course_project.instructors(id) ON DELETE RESTRICT,
    CONSTRAINT chk_course_courses_title_not_blank CHECK (char_length(trim(title)) > 0),
    CONSTRAINT chk_course_courses_level CHECK (level IN ('basic', 'intermediate', 'advanced')),
    CONSTRAINT chk_course_courses_price CHECK (price >= 0)
);
...
CREATE TABLE course_project.enrollments (
    ...
    CONSTRAINT fk_course_enrollments_student FOREIGN KEY (student_id)
        REFERENCES course_project.students(id) ON DELETE RESTRICT,
    CONSTRAINT fk_course_enrollments_course FOREIGN KEY (course_id)
        REFERENCES course_project.courses(id) ON DELETE RESTRICT,
    CONSTRAINT chk_course_enrollments_status CHECK (status IN ('신청', '수강중', '완료', '취소')),
    CONSTRAINT chk_course_enrollments_recorded_amount CHECK (recorded_amount >= 0)
);
...
BEGIN
    IF to_regclass('course_project.students') IS NULL
       ...
       OR to_regclass('course_project.uq_course_enrollments_active') IS NULL THEN
        RAISE EXCEPTION '스키마 생성 검증 실패: 프로젝트 객체가 모두 생성되지 않았습니다.';
    END IF;
    ...
    IF v_named_constraint_count <> 15 OR v_not_null_count <> 20
       OR v_student_count <> 0 OR v_instructor_count <> 0
       OR v_course_count <> 0 OR v_enrollment_count <> 0 THEN
        RAISE EXCEPTION
            '스키마 생성 검증 실패: named_constraints=%, not_null_columns=%, rows=%/%/%/%',
            v_named_constraint_count,v_not_null_count,v_student_count,v_instructor_count,v_course_count,v_enrollment_count;
    END IF;
