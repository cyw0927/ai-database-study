...
BEGIN
    IF current_database() <> 'ai_database_book' THEN
        RAISE EXCEPTION '변경 중단: 현재 데이터베이스는 %입니다. ai_database_book에 연결하세요.', current_database();
    END IF;

    IF current_setting('transaction_read_only')::boolean THEN
        RAISE EXCEPTION '변경 중단: 현재 연결이 읽기 전용입니다.';
    END IF;
    ...
    IF to_regclass('course_project.students') IS NULL
       ...
       OR to_regclass('course_project.uq_course_enrollments_active') IS NULL THEN
        RAISE EXCEPTION '변경 중단: 프로젝트 객체가 모두 존재하지 않습니다.';
    END IF;
    ...
    SELECT COUNT(*) INTO v_count_1005 FROM course_project.enrollments WHERE id = 1005;

    SELECT COUNT(*) INTO v_active_102_302
    FROM course_project.enrollments
    WHERE student_id = 102 AND course_id = 302 AND status IN ('신청', '수강중');

    SELECT price INTO v_course_302_price FROM course_project.courses WHERE id = 302;

    IF v_student_count <> 3 OR v_instructor_count <> 2 OR v_course_count <> 3 OR v_enrollment_count <> 4
       OR v_recorded_total <> 470000 OR v_status_1001 IS DISTINCT FROM '수강중'
       OR v_status_1004 IS DISTINCT FROM '신청' OR v_count_1005 <> 0
       OR v_active_102_302 <> 0 OR v_course_302_price IS DISTINCT FROM 120000::numeric THEN
        RAISE EXCEPTION
            '변경 시작 상태 불일치: rows=%/%/%/%, total=%, status1001=%, status1004=%, id1005=%, active102_302=%, course302_price=%',
            v_student_count,v_instructor_count,v_course_count,v_enrollment_count,v_recorded_total,
            v_status_1001,v_status_1004,v_count_1005,v_active_102_302,v_course_302_price;
    END IF;
END
$$;
...
SELECT 1005, 102, c.id, DATE '2026-04-07', '신청', c.price
...
SET status = '완료'
WHERE id = 1001 AND status = '수강중'
RETURNING id, status, recorded_amount;

UPDATE course_project.enrollments
SET status = '취소'
WHERE id = 1004 AND status = '신청'
RETURNING id, status, recorded_amount;

ALTER TABLE course_project.enrollments ALTER COLUMN id RESTART WITH 1006;
...
BEGIN
    ...
    INTO v_enrollment_count, v_recorded_total, v_non_cancelled_count, v_non_cancelled_recorded
    FROM course_project.enrollments;
    ...
    IF v_student_count <> 3 OR v_instructor_count <> 2 OR v_course_count <> 3 OR v_enrollment_count <> 5
       ...
       OR v_active_duplicate_count <> 0 OR v_recorded_total <> 590000
       OR v_non_cancelled_count <> 4 OR v_non_cancelled_recorded <> 440000 THEN
        RAISE EXCEPTION
            ...
            v_status_1001,v_amount_1001,v_status_1004,v_amount_1004,v_status_1005,v_amount_1005,
            v_active_duplicate_count,v_recorded_total,v_non_cancelled_count,v_non_cancelled_recorded;
    END IF;
